...
再生の直前に **マイクが稼働中か**を CoreAudio（`kAudioDevicePropertyDeviceIsRunningSomewhere`）で確認し、
使用中なら読み上げをスキップします（音声入力へのナレーター音の回り込み防止）。判定は標準ライブラリ
（`ctypes`）のみで行い、判定できない環境ではスキップせず通常どおり読み上げます（フェイルオープン）。
